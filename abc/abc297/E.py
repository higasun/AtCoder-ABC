...
class AVLTree:
    ...
    def delete(self, key):
        ...
        while history:
            ...
            if b == 2:
                if p.lch.bias == -1:
                    new_p = self.__rotateLR(p)
                else:
                    new_p = self.__rotate_right(p)

            elif b == -2:
                if p.rch.bias == 1:
                    new_p = self.__rotateRL(p)
                else:
                    new_p = self.__rotate_left(p)
...
crr = 0
while k:
    x = ss.lower_bound(crr+1)
    k -= 1
    ss.insert(2*x)


print(ss.find_kth_element(k-1))
